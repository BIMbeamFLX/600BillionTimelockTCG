...
| `NUTFT_INVOICE_TTL_SECONDS: must be a whole number of seconds, at least 60`, and the same for `NUTFT_CLAIM_GRACE_SECONDS`, `G_NUTFT_INVOICE_TTL_SECONDS`, `G_NUTFT_CLAIM_GRACE_SECONDS` | Set, and not written as a whole number of at least 60, such as `900.5` or `9e2`. | The running build started with these: it read a fraction, or a number in another notation, as its value (a fraction was held that long; phoenixd rounded the invoice expiry down, while lnd was sent the fraction as written). Write that number in digits, a fraction rounded to whole seconds, which moves the window by under a second. For G, `0` or a non-number took the E1 value, then 900 or 3600: write that. A value below 60 was refused before too. |
| `NUTFT_CLAIM_GRACE_SECONDS: must be at least NUTFT_INVOICE_TTL_SECONDS`, same for G | The claim grace is shorter than the quote window. | Unchanged: the running build refused this too, for G on the values it inherited. |
